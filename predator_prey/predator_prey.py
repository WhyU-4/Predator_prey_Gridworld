...
class PredatorPrey(gym.Env):
    ...
    def __init__(self, grid_shape=(5, 5), n_agents=2, n_preys=1, prey_move_probs=(0.175, 0.175, 0.175, 0.175, 0.3),
                 full_observable=False, penalty=-0.5, step_cost=-0.01, prey_capture_reward=50, max_steps=100,
                 agent_view_mask=(9, 9)):
...
# 下上右左
    ...
    def __get_view_coordinates(self, pos, view_range=1):
        views = []
    ...
    def render(self, mode='human'):
        ...
        for agent_i in range(self.n_agents):
            ...
            for view in self.__get_view_coordinates(self.agent_pos[agent_i]):
                fill_cell(img, view, cell_size=CELL_SIZE, fill=VIEW_COLOR, margin=0.1)
...
VIEW_COLOR =  ImageColor.getcolor('green', mode='RGB')
